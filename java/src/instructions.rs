...
impl Ins {
    ...
    pub fn size(&self, curr: usize) -> usize {
        match self {
            Ins::Nop => 1,
            Ins::AConstNull => 1,
            Ins::IConstM1 => 1,
            ...
            Ins::Jsr { .. } => 3,
            Ins::Ret { .. } => 2,
            Ins::TableSwitch { offsets, .. } => {
                let mut pad = (curr + 1) % 4;
                if pad != 0 {
                    pad = 4 - pad;
                }

                1 + 4*3 + 4*offsets.len() + pad
            },
            Ins::LookupSwitch { pairs, .. } => {
                let mut pad = (curr + 1) % 4;
                if pad != 0 {
                    pad = 4 - pad;
                }

                1 + 4*2 + 4*pairs.len() + pad
            },
            Ins::IReturn => 1,
            Ins::LReturn => 1,
            Ins::FReturn => 1,
